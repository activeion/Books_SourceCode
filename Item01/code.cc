...
int main(void)
{
...

    {
        ...
        f2(cx); // T 的类型为const int, ParamType的类型为const int&
        f2(rx); // T 的类型为const int, ParamType的类型为const int&
    }

    {
        ...
        const int& rx = x;
        // 一样调用模板函数
        f3(&x);  // T 的类型为int, ParamType的类型为cont int*
...

    {
        ...
        f4(x);  // x是左值，所以T 和ParamType会被推断为int &类型
